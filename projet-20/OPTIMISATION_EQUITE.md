...
## ⚠️ État Actuel : CONTRAINTE DÉSACTIVÉE

### Statut

La contrainte d'équité domicile/extérieur est **actuellement désactivée** car elle cause des conflits (UNSAT) avec les autres contraintes du modèle.

### Problème Rencontré

Lorsque la contrainte d'équité stricte était activée, le solveur CP-SAT retournait immédiatement "Aucune solution trouvée" (en 1-2 secondes au lieu du timeout de 65 secondes), indiquant un conflit de contraintes (UNSAT - Unsatisfiable).

### Impact

Sans la contrainte d'équité, certaines instances peuvent présenter un **déséquilibre important** :

**Exemple observé (43 équipes, mode simple) :**

```
Équipe 1  : 83 domicile, 3 extérieur  ❌ TRÈS DÉSÉQUILIBRÉ
Équipe 43 : 0 domicile, 86 extérieur  ❌ TRÈS DÉSÉQUILIBRÉ
```
...
### Tentative 1 : Contrainte Stricte (ÉCHEC)

```python
# DÉSACTIVÉ - Cause UNSAT
...
**Problème :** Conflit avec les contraintes de breaks et max déplacements consécutifs.

### Tentative 2 : Tolérance Élargie (ÉCHEC)

Même avec une tolérance de ±2 matchs, le solveur ne trouve pas de solution.

### Solution Future : Contrainte Souple (Soft Constraint)

Au lieu d'une contrainte **dure** (hard constraint) qui doit être satisfaite, implémenter une contrainte **souple** dans la fonction objectif :

```python
# Fonction objectif multi-critères
objective = (
    weight_breaks * sum(breaks)
    + weight_equity * sum(abs(home_count - target) for each team)
)
...
- ✅ Optimiser l'équité autant que possible
- ✅ Faire des compromis entre breaks et équité

---

## 📊 État Actuel du Code

Dans [`src/model.py`](src/model.py) (lignes 74-77) :
