...
## 🐛 Problèmes connus et améliorations à venir

### ⚠️ Issues en cours

1. **Contrainte d'équité domicile/extérieur** : Actuellement désactivée car elle cause des conflits (UNSAT) avec les autres contraintes.

   - **Impact** : Certaines équipes peuvent avoir un déséquilibre important (ex: 83 domiciles / 3 extérieurs)
   - **Solution prévue** : Implémenter comme contrainte souple (soft constraint) dans la fonction objectif au lieu de contrainte dure

2. **Calcul des breaks** : Le calcul inclut maintenant les breaks même avec des byes entre deux matchs du même type

   - **Comportement actuel** : D-BYE-D compte comme un break
   - **Alternative** : Peut être modifié pour ne compter que les matchs strictement consécutifs

3. **Performance avec grandes instances** : Pour 20+ équipes, le temps de résolution peut dépasser le timeout
   - **Recommandation** : Augmenter max_time à 120-300 secondes pour les grands tournois

### 🔧 Améliorations futures

- [ ] Implémenter l'équité comme contrainte souple (weighted objective)
- [ ] Ajouter des contraintes de disponibilité de stades
- [ ] Support des tournois multi-divisions
- [ ] Export PDF du calendrier
- [ ] Mode "recherche rapide" avec heuristiques simplifiées
